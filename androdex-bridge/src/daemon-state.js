...
const DEFAULT_STATE_DIR_NAME = ".androdex";
...
function resolveAndrodexStateDir({ env = process.env, osImpl = os } = {}) {
  return normalizeNonEmptyString(env.ANDRODEX_DEVICE_STATE_DIR)
    || path.join(osImpl.homedir(), DEFAULT_STATE_DIR_NAME);
}

function resolveDaemonConfigPath(options = {}) {
  return path.join(resolveAndrodexStateDir(options), DAEMON_CONFIG_FILE);
}

function resolvePairingSessionPath(options = {}) {
  return path.join(resolveAndrodexStateDir(options), PAIRING_SESSION_FILE);
}

function resolveBridgeStatusPath(options = {}) {
  return path.join(resolveAndrodexStateDir(options), BRIDGE_STATUS_FILE);
}

function resolveBridgeLogsDir(options = {}) {
  return path.join(resolveAndrodexStateDir(options), LOGS_DIR);
}
...
function ensureAndrodexStateDir({ fsImpl = fs, ...options } = {}) {
  fsImpl.mkdirSync(resolveAndrodexStateDir(options), { recursive: true });
}

function ensureAndrodexLogsDir({ fsImpl = fs, ...options } = {}) {
