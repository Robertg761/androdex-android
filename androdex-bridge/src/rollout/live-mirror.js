...
function synthesizeNotificationsFromRolloutEntry(entry, state) {
  ...
  if (entry?.type === "event_msg") {
    return eventMessageNotifications(state, entry.payload || {});
  }

  if (entry?.type === "response_item") {
    return responseItemNotifications(state, entry.payload || {});
  }

  return [];
}

function eventMessageNotifications(state, payload) {
  ...
  if (eventType === "agent_message") {
    if (!shouldMirrorAgentMessage(payload)) {
      return [];
    }

    const message = readString(payload.message);
    if (!message || !state.activeTurnId) {
      return [];
    }
...
function responseItemNotifications(state, payload) {
  const itemType = readString(payload.type);
  if (itemType === "function_call") {
    return toolCallNotifications(state, payload);
  }
  if (itemType === "function_call_output") {
    return toolOutputNotifications(state, payload);
  }
  return [];
}

function toolCallNotifications(state, payload) {
  if (!state.activeTurnId) {
    return [];
  }
  ...
  const cwd = resolveToolWorkingDirectory(argumentsObject, state);
...
function isDesktopRolloutOrigin(sessionMeta) {
  ...
  if (originator.includes("mobile") || originator.includes("android") || originator.includes("ios")) {
    return false;
  }
