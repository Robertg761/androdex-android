// FILE: git-handler.js
// Purpose: Intercepts git/* JSON-RPC methods and executes git commands locally on the host.
...
async function stashChangesForWorktreeHandoff(cwd) {
  const stashLabel = `androdex-worktree-handoff-${randomBytes(6).toString("hex")}`;
...
async function applyCopiedLocalChangesToWorktree(cwd, patch) {
  ...
  const patchFilePath = path.join(os.tmpdir(), `androdex-worktree-copy-${randomBytes(6).toString("hex")}.patch`);
...
async function resolveGitCwd(params) {
  ...
  if (!isExistingDirectory(requestedCwd)) {
    throw gitError(
      "missing_working_directory",
      "The requested local working directory does not exist on this host."
    );
  }
