...
- `data/AndrodexPersistence.kt`: persistent storage for the disposable saved relay session plus the durable phone identity, trusted host registry, recovery payloads, thread runtime overrides, cached timeline/history data, and pairing-critical synchronous writes that must survive process death.
...
- `src/bridge.js`: main bridge process that connects the relay, secure transport, Codex runtime, notifications, workspace runtime, git/workspace handlers, desktop refresh logic, initialize-replay logic after host transport restarts, and the stable public host route used while private relay transport sessions rotate underneath.
...
- `relay.js`: core in-memory WebSocket relay that pairs one host daemon with one mobile client, handles heartbeat/liveness, maps the stable public host route onto the current live Mac session, supports trusted-session resolution, and verifies trusted recovery/rekey requests against current and fallback recovery identities.
