...
- `androdex status` now also surfaces installed T3 desktop/CLI discovery, any live local desktop session endpoint it can see, and whether the local runtime-session descriptor currently looks trusted or stale
...
- the local T3 source tree now has that compatibility hook drafted in desktop source as a permission-restricted `~/.t3/userdata/runtime-session.json` descriptor, and the Androdex bridge is now ready to prefer that descriptor over log scraping when it is present
- the bridge now only trusts that runtime-session descriptor when it validates as a loopback websocket session and, when a backend pid is present, that pid still looks alive; otherwise Androdex falls back cleanly and `androdex doctor` explains the descriptor problem instead of silently attaching to stale state
- the bridge test suite now also has a focused end-to-end coverage slice for this installed-desktop path, proving a trusted desktop runtime-session descriptor flows from `readBridgeConfig()` into workspace runtime activation and into the live T3 adapter with the auth token kept separate from the printed endpoint
- T3 desktop discovery now consistently honors the explicit bridge environment home directory instead of implicitly falling back to the parent process home path, so config/status/doctor all resolve the same descriptor and log files under the selected host state root
...
- workspace activation: supported
- `thread/list` / `thread/read`: supported
- `thread/resume`: supported
- live turn and timeline updates: supported
- send / plan / subagent flows: supported
- interrupt: supported
- approval responses: supported
- tool and user-input responses: supported
- rollback / compaction / background terminal cleanup: supported when the current host bridge build advertises them
